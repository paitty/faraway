...
from faraway.data_structures import MainCard
from faraway.player_field import PlayerField
from faraway.load_cards import load_main_cards
...
def create_image_from_maincard(
    ...
    # size top-right asset icons larger for visibility
    icon_size = int(card_size * 0.12)
    icon_spacing = int(card_size * 0.035)
    icons_to_draw = []
...
def create_card_image(
    obj: Union[MainCard, PlayerField],
...
def create_card_image_from_maincard(
    card: MainCard,
    out_path: str = "card.png",
    card_size: int = DEFAULT_CARD_SIZE,
    font_path: Optional[str] = None,
):
    """Deprecated: Use create_card_image() instead."""
    return create_image_from_maincard(card, out_path=out_path, card_size=card_size, font_path=font_path)
...
if __name__ == "__main__":
    ...
    player_field = PlayerField(main_cards=player_cards)
